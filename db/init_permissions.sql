...
-- =============================================================
-- STEP 2: Rebuild the dotpermissions lookup table.
-- This replicates what regeneratePermissions() does in PHP.
-- Must run AFTER all gacl tables are populated above.
-- =============================================================

TRUNCATE TABLE `dotp_dotpermissions`;

-- Pass 1: Direct ARO → AXO (individual user → specific module object)
INSERT INTO `dotp_dotpermissions`
    (acl_id, user_id, section, axo, permission, allow, priority, enabled)
SELECT acl.id, aro.value, axo_m.section_value, axo_m.value,
       aco_m.value, acl.allow, 1, acl.enabled
FROM `dotp_gacl_acl` acl
LEFT JOIN `dotp_gacl_aco_map`  aco_m ON acl.id = aco_m.acl_id
LEFT JOIN `dotp_gacl_aro_map`  aro_m ON acl.id = aro_m.acl_id
LEFT JOIN `dotp_gacl_aro`      aro   ON aro_m.value = aro.value
LEFT JOIN `dotp_gacl_axo_map`  axo_m ON axo_m.acl_id = acl.id
WHERE aro.name IS NOT NULL AND axo_m.value IS NOT NULL;

-- Pass 2: Direct ARO → AXO groups (individual user → module group)
INSERT INTO `dotp_dotpermissions`
    (acl_id, user_id, section, axo, permission, allow, priority, enabled)
...
LEFT JOIN `dotp_gacl_aro_map`        aro_m  ON acl.id = aro_m.acl_id
LEFT JOIN `dotp_gacl_aro`            aro    ON aro_m.value = aro.value
LEFT JOIN `dotp_gacl_axo_groups_map` axo_gm ON axo_gm.acl_id = acl.id
LEFT JOIN `dotp_gacl_axo_groups`     axo_g  ON axo_gm.group_id = axo_g.id
LEFT JOIN `dotp_gacl_groups_axo_map` g_axo_m ON axo_g.id = g_axo_m.group_id
LEFT JOIN `dotp_gacl_axo`            axo    ON g_axo_m.axo_id = axo.id
WHERE aro.value IS NOT NULL AND axo_g.value IS NOT NULL;

-- Pass 3: ARO groups → AXO (role group → specific module object)
INSERT INTO `dotp_dotpermissions`
    (acl_id, user_id, section, axo, permission, allow, priority, enabled)
SELECT acl.id, aro.value, axo_m.section_value, axo_m.value,
       aco_m.value, acl.allow, 3, acl.enabled
FROM `dotp_gacl_acl` acl
LEFT JOIN `dotp_gacl_aco_map`        aco_m  ON acl.id = aco_m.acl_id
LEFT JOIN `dotp_gacl_aro_groups_map` aro_gm ON acl.id = aro_gm.acl_id
LEFT JOIN `dotp_gacl_aro_groups`     aro_g  ON aro_gm.group_id = aro_g.id
LEFT JOIN `dotp_gacl_axo_map`        axo_m  ON axo_m.acl_id = acl.id
LEFT JOIN `dotp_gacl_groups_aro_map` g_aro_m ON aro_g.id = g_aro_m.group_id
LEFT JOIN `dotp_gacl_aro`            aro    ON g_aro_m.aro_id = aro.id
WHERE axo_m.value IS NOT NULL AND aro.name IS NOT NULL;

-- Pass 4: ARO groups → AXO groups (role group → module group — the main admin grant)
INSERT INTO `dotp_dotpermissions`
    (acl_id, user_id, section, axo, permission, allow, priority, enabled)
SELECT acl.id, aro.value, axo.section_value, axo.value,
       aco_m.value, acl.allow, 4, acl.enabled
FROM `dotp_gacl_acl` acl
LEFT JOIN `dotp_gacl_aco_map`        aco_m   ON acl.id = aco_m.acl_id
LEFT JOIN `dotp_gacl_aro_map`        aro_m   ON acl.id = aro_m.acl_id
LEFT JOIN `dotp_gacl_aro_groups_map` aro_gm  ON acl.id = aro_gm.acl_id
...
-- All done. The admin user should now have full access to all modules.
