...
if (
	isset($_GET['update_project_status']) && isset($_GET['project_status'])
...
$bufferUser = arraySelect(
	$userRows,
	'show_owner',
	'class="text" onchange="javascript:document.pickUser.submit()""',
	$owner
);
...
foreach ($project_types as $psk => $project_status) {
	$tabBox->add(
		$project_status_file[$psk],
		(isset($project_status_tabs[$psk]) ? $project_status_tabs[$psk] : $AppUI->_($project_status)),
		true,
		$psk
	);
}
