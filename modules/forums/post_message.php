...
<table cellspacing="0" cellpadding="3" border="0" width="98%" class="std">
	...
	<form name="changeforum" action="?m=forums&amp;forum_id=<?php echo $forum_id; ?>" method="post">
		...
		<input type="hidden" name="message_published"
			value="<?php echo $forum_info['forum_moderated'] ? '1' : '0'; ?>" />
		<input type="hidden" name="message_author"
			value="<?php echo (isset($message_info['message_author']) && ($message_id || $message_parent < 0)) ? $message_info["message_author"] : $AppUI->user_id; ?>" />
		<input type="hidden" name="message_editor"
			value="<?php echo (isset($message_info['message_author']) && ($message_id || $message_parent < 0)) ? $AppUI->user_id : '0'; ?>" />
		...
		<table cellspacing="0" cellpadding="3" border="0" width="98%" class="std">
			...
			if ($message_parent >= 0) {
				...
				<tr>
					...
					<td align="left"><?php echo $AppUI->___($message_info['message_title']); ?></td>
				</tr>
				<tr>
					<td align="right" valign="top"><?php echo $AppUI->_('Message') ?>:</td>
					<td align="left"><textarea name="message_parent_body" cols="60" readonly="readonly"
							style="height:100px; font-size:8pt"><?php echo $AppUI->___($message_info['message_body']); ?></textarea>
					</td>
				</tr>
				<tr>
			...
			<tr>
				...
				<td>
					<input type="text" name="message_title"
						value="<?php echo ($message_id || $message_parent < 0 ? '' : 'Re: ') . $AppUI->___(isset($message_info['message_title']) ? $message_info['message_title'] : ''); ?>"
			...
			<tr>
				...
				<td align="left" valign="top">
					<textarea cols="60" name="message_body"
						style="height:200px"><?php echo $AppUI->___((($message_id == 0) and ($message_parent != -1)) ? "\n>" . (isset($last_message_info['message_body']) ? $last_message_info['message_body'] : '') . "\n" : (isset($message_info['message_body']) ? $message_info['message_body'] : '')); ?></textarea>
				</td>
			</tr>
			<tr>
				<td>
				...
				<td align="right">
					<?php
					if (
						$canEdit && ($forum_info['forum_owner'] == $AppUI->user_id
