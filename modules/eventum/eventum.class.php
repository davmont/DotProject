...
class CEventumConfig
{
	...
	{
		return isset($this->config[$config_name]) ? $this->config[$config_name] : null;
	}
	...
	function setValue($config_name, $config_value)
	{
		...
		if (isset($this->config[$config_name]) && $this->config[$config_name] !== null) {
			// Update config
...
class CEventumSupportLevel extends CDpObject
{
	...
	{
		if (
			$this->support_level_desc != NULL &&
			$this->support_minresponse_hrs != NULL &&
			$this->support_maxresponse_hrs != NULL
		)
			return true;
		return false;
	}
	...
	function delete($del_id = null, $history_desc = '', $history_proj = 0)
	{
		$q = new DBQuery;
