...
function displayFolders($folder_id = 0, $level = 0)
{
	...
	global $company_id, $project_id, $task_id, $folder;
	global $allowedCompanies, $allowedProjects, $allowedTasks, $allowedFolders, $allowedFolderIDs;
	...
	foreach ($folders as $row) {
		...
		echo ((($m == 'files')
			? ('<a' . (($folder_id)
				...
				. '">') : '') . "\n");
		echo (dPshowImage(
			DP_BASE_URL . '/modules/files/images/folder5_small.png',
			'16',
			'16',
			'folder icon',
			...
			if ($folder_id) {
				...
				if ($canEdit_this) {
					echo ('<a href="?m=files&amp;a=addedit_folder&amp;folder=' . $folder_id
						. '">' . dPshowImage(
						DP_BASE_URL . '/modules/files/images/kedit.png',
						'16',
						'16',
						'edit icon',
						'edit this folder'
				...
				if ($canAuthor_this) {
					echo ('<a href="?m=files&amp;a=addedit_folder&amp;file_folder_parent='
						. $folder_id . '&amp;folder=0">'
						. dPshowImage(
							DP_BASE_URL . '/modules/files/images/edit_add.png',
							'16',
							'16',
							'new folder',
							$AppUI->_('add a new subfolder')
						)
						. '</a>');
				}
				if ($canDelete_this) {
					...
					echo ('<a href="#" onclick="javascript:delCheck(' . "'" . $folder_id . "'"
						. ')">' . dPshowImage(
						DP_BASE_URL . '/modules/files/images/remove.png',
						'16',
						'16',
						'delete icon',
				...
				echo ('<a href="?m=files&amp;a=addedit&amp;folder=' . $folder_id
					...
					. dPshowImage(
						DP_BASE_URL . '/modules/files/images/folder_new.png',
						'16',
						'16',
						'new file',
						$AppUI->_('add new file to this folder')
					)
					. '</a>');
				echo ("</td>\n");
				echo ("</tr></table>\n");
	...
	if (is_array($allowedFolderIDs) && count($allowedFolderIDs)) {
...
function countFiles($folder_id)
{
	...
	if (is_array($allowedFolders) && count($allowedFolders)) {
		$q->addWhere('((' . implode(' AND ', $allowedFolders) . ') OR f.file_folder = 0)');
	}
	if (is_array($allowedProjects) && count($allowedProjects)) {
		$q->addWhere('((' . implode(' AND ', $allowedProjects) . ') OR f.file_project = 0)');
	}
	if (is_array($allowedTasks) && count($allowedTasks)) {
	...
	if ($company_id) {
		...
		if (is_array($allowedCompanies) && count($allowedCompanies)) {
			...
			<tr>
				...
				<td width="15%">
					...
					if (
						...
						echo $row['file_id']; ?>">
							<?php
							echo dPshowImage(
								(DP_BASE_URL . '/images/icons/stock_cancel-16.png'),
								'16',
								'16',
								'cancel checkout',
								'cancel file checkout'
							);
							?>
						</a><?php
					} ?>
				...
				<td nowrap="nowrap" align="center">
					...
					if ($canEdit && empty($row['file_checkout'])) {
						...
						<a href="?m=files&amp;a=co&amp;file_id=<?php echo $row['file_id']; ?>">
							<?php
							echo dPshowImage(
								DP_BASE_URL . '/modules/files/images/co.png',
								'16',
								'16',
								'checkout',
								'checkout file'
						...
						?>
							<a href="?m=files&amp;a=addedit&amp;ci=1&amp;file_id=<?php echo $row['file_id']; ?>">
								<?php
								echo dPshowImage(
									DP_BASE_URL . '/modules/files/images/ci.png',
									'16',
									'16',
									'checkin',
									'checkin file'
				...
				<td nowrap="nowrap" align="right" width="48">
					...
					if (empty($row['file_checkout']) || $row['file_checkout'] == 'final') {
						// Edit File
						if (
							$canEdit && (empty($row['file_checkout'])
						...
						if ($canAuthor || $row['project_owner'] == $AppUI->user_id) {
							...
							echo $row['file_id']; ?>.submit()">
								<?php
								echo (dPshowImage(
									DP_BASE_URL . '/modules/files/images/duplicate.png',
									'16',
									'16',
									'duplicate file',
									'duplicate file'
				...
				<td nowrap="nowrap" align="center" width="1">
					<?php
					if (
						(empty($row['file_checkout']) || $row['file_checkout'] == 'final')
									...
									<tr>
										...
										<td nowrap="nowrap" align="right" width="48">
											...
											if (empty($file['file_checkout']) || $file['file_checkout'] == 'final') {
												// Edit File
												if (
													$canEdit && $dPconfig['files_show_versions_edit']
												...
												// Delete File
												if (
													$canDelete && $dPconfig['files_show_versions_edit']
													&& ($canAdmin || $file['project_owner'] == $AppUI->user_id
														|| $file['file_co_reason'] == '')
												) {
													?>
		...
		<tr>
			<td nowrap="nowrap">
				...
				echo ("\t\t\t" . '<a href="?m=' . $m . '&amp;a=' . $a . '&amp;tab=' . $tab . '&amp;folder=0">'
					. "\n");
				echo ("\t\t\t" . dPshowImage(
					DP_BASE_URL . '/modules/files/images/home.png',
					'22',
					'22',
					'folder icon',
					$AppUI->_('back to root folder')
				) . "\n");
				echo ("\t\t\t" . '</a>' . "\n");
				...
				if ($canRead_parent) {
					...
					echo ("\t\t\t" . dPshowImage(
						DP_BASE_URL . '/modules/files/images/back.png',
						'22',
						'22',
						'folder icon',
						$AppUI->_('back to parent folder')
					) . "\n");
					echo ("\t\t\t" . '</a>' . "\n");
				}
...
//get list of allowed projects
	$project = new CProject();
	$projects_list = $project->getAllowedRecords(
		$AppUI->user_id,
	...
	$folders_list = arrayMerge(
		array('O' => array('O', ('(' . $AppUI->_('Move to Folder') . ')'), -1)),
		$folders_list
	);
	...
	$projects_list = arrayMerge(
		array(
			'O' => ('(' . $AppUI->_('Move to Project') . ')'),
