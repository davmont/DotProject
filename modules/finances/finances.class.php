...
class CBudget extends CDpObject
{
	...
	{
		if (countChildren($this->task_id) != 0)
			return false;
		if ($this->task_id <= 0)
			return false;
		$q = new DBQuery;
	...
	function get_equipment_investment($mult = 1, $symbol = "", $sep = " ")
	{
		$tax = 1;
		if ($this->display_tax)
			$tax += ($this->Tax / 100);
		return number_format($this->equipment_investment * $tax * $mult, 2, '.', $sep) . $symbol;
	}
	function get_intangible_investment($mult = 1, $symbol = "", $sep = " ")
	{
		$tax = 1;
		if ($this->display_tax)
			$tax += ($this->Tax / 100);
		return number_format($this->intangible_investment * $tax * $mult, 2, '.', $sep) . $symbol;
	}
	function get_service_investment($mult = 1, $symbol = "", $sep = " ")
	{
		$tax = 1;
		if ($this->display_tax)
			$tax += ($this->Tax / 100);
		return number_format($this->service_investment * $tax * $mult, 2, '.', $sep) . $symbol;
	}
	function get_equipment_operation($mult = 1, $symbol = "", $sep = " ")
	{
		$tax = 1;
		if ($this->display_tax)
			$tax += ($this->Tax / 100);
		return number_format($this->equipment_operation * $tax * $mult, 2, '.', $sep) . $symbol;
	}
	function get_intangible_operation($mult = 1, $symbol = "", $sep = " ")
	{
		$tax = 1;
		if ($this->display_tax)
			$tax += ($this->Tax / 100);
		return number_format($this->intangible_operation * $tax * $mult, 2, '.', $sep) . $symbol;
	}
	function get_service_operation($mult = 1, $symbol = "", $sep = " ")
	{
		$tax = 1;
		if ($this->display_tax)
			$tax += ($this->Tax / 100);
		return number_format($this->service_operation * $tax * $mult, 2, '.', $sep) . $symbol;
	}
	...
	function bestCurrency()
	{
		...
		if ($this->equipment_investment != 0) {
			if ($this->equipment_investment <= 1000000)
				$ret = 1;
			if ($this->equipment_investment <= 1000)
				return 0;
		}
		if ($this->intangible_investment != 0) {
			if ($this->intangible_investment <= 1000000)
				$ret = 1;
			if ($this->intangible_investment <= 1000)
				return 0;
		}
		if ($this->service_investment != 0) {
			if ($this->service_investment <= 1000000)
				$ret = 1;
			if ($this->service_investment <= 1000)
				return 0;
		}
		if ($this->equipment_operation != 0) {
			if ($this->equipment_operation <= 1000000)
				$ret = 1;
			if ($this->equipment_operation <= 1000)
				return 0;
		}
		if ($this->intangible_operation != 0) {
			if ($this->intangible_operation <= 1000000)
				$ret = 1;
			if ($this->intangible_operation <= 1000)
				return 0;
		}
		if ($this->service_operation != 0) {
			if ($this->service_operation <= 1000000)
				$ret = 1;
			if ($this->service_operation <= 1000)
				return 0;
		}
		return $ret;
	}
	...
	function isNull()
	{
		if ($this->equipment_investment + $this->intangible_investment + $this->service_investment + $this->equipment_operation + $this->intangible_operation + $this->service_operation > 0)
			return false;
		else
			return true;
	}
...
function updateValue($var, $val, $tax)
{
	...
	switch ($type) {
		case "ei":
			$q->addUpdate('equipment_investment', $val);
			break;
		case "ii":
			$q->addUpdate('intangible_investment', $val);
			break;
		case "si":
			$q->addUpdate('service_investment', $val);
			break;
		case "eo":
			$q->addUpdate('equipment_operation', $val);
			break;
		case "io":
			$q->addUpdate('intangible_operation', $val);
			break;
		case "so":
			$q->addUpdate('service_operation', $val);
			break;
		default:
			return -1;
	}
