				...
				<td align='right'>
					<table>
						<tr>
							...
							<td>
								<input type="checkbox" name="show_incomplete" id="show_incomplete"
	...
	foreach ($projects as $k => $p) {
		...
		if ($tnums > 0 || $project_id == $p['project_id']) {
			...
			if (!$min_view) {
				// not minimal view
				?>
				<form name="assFrm<?php echo ($p['project_id']) ?>" action="index.php?m=<?php echo ($m); ?>&amp;a=<?php echo ($a); ?>"
					method="post">
						...
						?>
							<tr>
								<td colspan="<?php echo $cols ?>">
									<a href="?m=projects&amp;a=view&amp;project_id=<?php echo $t1['task_project']; ?>"
										class="hdr"><b><?php echo $AppUI->_('Project'); ?> : <?php echo $p['project_name'] ?></b></a>
								</td>
			...
			?>
				<tr>
					...
					<td><?php $summary_date = new CDate($summaries['start_date']);
					echo $summary_date->format($df); ?></td>
					<td align="center"><?php echo $summaries['duration'] ?> 		<?php echo $AppUI->_('hours'); ?></td>
					<td><?php $summary_date = new CDate($summaries['end_date']);
					echo $summary_date->format($df); ?></td>
					<td></td>
				</tr>
				<?php
				if ($tnums && $dPconfig['enable_gantt_charts'] && !$min_view) {
					...
					<tr>
						<td colspan="<?php echo $cols; ?>" align="right">
							<a
								href="<?php echo 'index.php' . $query_string . '&amp;open_task_all=1'; ?>"><?php echo $AppUI->_('Expand All'); ?></a>
							:
							<a
								href="<?php echo 'index.php' . $query_string . '&amp;close_task_all=1'; ?>"><?php echo $AppUI->_('Collapse All'); ?></a>
