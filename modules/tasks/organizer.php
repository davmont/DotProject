...
function fixate_task($task_index, $time, $dep_on_task)
{
	...
	foreach ($tasks as $task2) {
		...
		if (
			...
			if (!$option_advance_if_possible || $task2["task_percent_complete"] != 100) {
				...
				if (isset($option_check_vital_users) && $option_check_vital_users) {
					$q = new DBQuery;
	...
	if (
		db_dateTime2unix($tasks[$task_index]["task_start_date"]) == $start_date &&
...
<table name="table" cellspacing="1" cellpadding="1" border="0" width="100%">
	<tr>
		...
		<td nowrap>
			<h1><?php echo $AppUI->_('Tasks Organizer Wizard'); ?></h1>
		</td>
	...
	if ($do != "conf") {
		...
		while ($row = $q->fetchRow()) {
			...
			if (!$row["task_duration"] && $row["task_end_date"] == $NO_DATE) {
				log_error(
					"Task " . task_link($row) . " has no duration.",
			...
			if (!$row["task_dynamic"] && $row["task_end_date"] == $NO_DATE) {
				...
				if ($do == "ask" && $option_no_end_date_warning) {
					log_warning(
						"Task " . task_link($row) . " has no end date. Using tasks duration instead.",
				...
				if (!$row["task_dynamic"] && $row["task_percent_complete"] == 0) {
					...
					if ($end_time < time()) {
						if ($option_check_delayed_tasks) {
							log_warning(
								"Task " . task_link($row) . " started on " . $row["task_start_date"] . " and ended on " . formatTime($end_time) . ".",
		...
		if ($option_fix_task_group_date_ranges) {
			...
			while ($tg = $q->fetchRow()) {
				...
				if (
					db_dateTime2unix($tg["task_start_date"]) != $min_time
