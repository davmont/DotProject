...
function getEventLinks($startPeriod, $endPeriod, &$links, $strMaxLen)
{
	...
	$evt = new CEvent();
	$events = $evt->getEventsForPeriod($startPeriod, $endPeriod, $event_filter);
	...
	foreach ($events as $row) {
		...
		for ($i = 0, $x = $end->dateDiff($start); $i <= $x; $i++) {
			...
			if (
				($row['event_cwd'] && in_array($date->getDayOfWeek(), $cwd))
				|| !($row['event_cwd'])
			) {
				$link['href'] = '?m=calendar&a=view&event_id=' . $row['event_id'];
				$link['alt'] = $row['event_description'];
				$link['text'] = (dPshowImage(dPfindImage(
					('event' . $row['event_type'] . '.png'),
					'calendar'
				), 16, 16)
