...
if (!$row->load($contact_id) && $contact_id > 0) {
	...
	$row->contact_private && $row->contact_owner != $AppUI->user_id
...
$titleBlock->addCell(
	...
	('<form action="?m=projects&amp;a=addedit&amp;company_id=' . $row->contact_company
		. '&amp;contact_id=' . $contact_id . '" method="post">'),
	'</form>'
);
			...
			<table border="0" cellpadding="1" cellspacing="1" class="details" width="100%">
				...
				if (isset($_SESSION['all_tabs']['departments'])) {
					...
					<tr>
						...
						<td class="hilite" nowrap="nowrap"><?php echo $AppUI->___($dept_detail['dept_name']); ?></td>
					</tr>
				<?php } ?>
				<tr>
				...
				<tr>
					...
					<td class="hilite" nowrap="nowrap"><a href="mailto:<?php echo $AppUI->___(@$row->contact_email); ?>"><?php
					  echo $AppUI->___(@$row->contact_email); ?></a></td>
				</tr>
				<tr>
					<td align="right"><?php echo $AppUI->_('Email'); ?>2:</td>
					<td class="hilite" nowrap="nowrap"><a
							href="mailto:<?php echo $AppUI->___(@$row->contact_email2); ?>"><?php
				...
				<tr>
					...
					<td class="hilite" nowrap="nowrap"><?php echo @mb_substr($row->contact_birthday, 0, 10); ?></td>
				</tr>
			</table>
		</td>
		<td valign="top" width="50%">
	...
	<tr>
		<td>
			<input type="button" value="<?php echo $AppUI->_('back'); ?>" class="button"
				onclick="javascript:window.location='./index.php?m=contacts';" />
		</td>
	</tr>
</table>