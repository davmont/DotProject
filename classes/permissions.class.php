...
class dPacl extends gacl_api
{
	...
	function checkModuleItem($module, $op, $item = null, $userid = null)
	{
		...
		if (isset($item)) {
			...
			if ($module == 'projects') {
				...
				if ($task_owner == $GLOBALS['AppUI']->user_id) {
					$IsTaskOwner = true;
				}
				// Task participant
				$q = new DBQuery;
				...
				foreach ($assigned_users as $val) {
					if ($val['user_id'] == $GLOBALS['AppUI']->user_id) {
				...
				$q->clear();
				foreach ($isAllowed as $allowToUser) {
					if ($allowToUser['permission'] == 'edit') {
						$IsProjectModifier = true;
					}
				}
			}
			// Grab owner
			if (isset($project_id)) {
				...
				if ($project_owner == $GLOBALS['AppUI']->user_id) {
					$IsProjectOwner = true;
				}
			}
			if (isset($macroproject_id)) {
				...
				if ($macroproject_owner == $GLOBALS['AppUI']->user_id) {
					$IsMacroProjectOwner = true;
				}
			}
			// Rules
			$itemowner = false;
			...
			if ($module == 'macroprojects' && $IsMacroProjectOwner) {
				$itemowner = true;
			}
			if ($module == 'tasks' && ($IsProjectOwner || $IsTaskOwner || $IsTaskParticipant || $IsProjectModifier)) {
				$itemowner = true;
			}
			if ($module == 'task_log' && ($IsProjectOwner || $IsTaskOwner || $IsTaskParticipant || $IsProjectModifier)) {
				$itemowner = true;
			}
		}
		// end of hack
	...
	function addUserPermission()
	{
		...
		if ($mod_type == 'grp,') {
			...
			if (
				!($this->get_object_id(
					$_POST['permission_table'],
					$_POST['permission_item'],
					'axo'
				))
			) {
				$this->addModuleItem(
