...
class CAppUI
{
	...
	function readDirs($path)
	{
		...
		while (false !== ($name = $d->read())) {
			if (
				is_dir(DP_BASE_DIR . '/' . $path . '/' . $name) && $name != '.' && $name != '..'
				&& $name != 'CVS' && $name != '.svn'
			) {
				$dirs[$name] = $name;
			}
	...
	function setUserLocale($loc = '', $set = true)
	{
		...
		list($base_locale, $english_string, $native_string, $default_language, $lcs) = array_pad($lang, 5, null);
		if (!isset($lcs)) {
	...
	function findLanguage($language, $country = false)
	{
		...
		foreach ($LANGUAGES as $lang => $info) {
			list($l, $c) = array_pad(explode('_', $lang, 2), 2, null);
			if ($l == $language) {
				if (!($first_entry)) {
					$first_entry = $lang;
				}
	...
	function setBaseLocale($context = LC_ALL)
	{
		...
		list($locale, $en_name, $local_name, $win_locale, $lcs) = array_pad($LANGUAGES['en_AU'], 5, null);
		$real_locale = 'en_AU';
	...
	function __($str, $flags = 0)
	{
		...
		if ($x) {
			...
			dPgetConfig('locale_warn') && !($this->base_locale == $this->user_locale
...
class CTabBox_core
{
	...
	function findTabModule($tab)
	{
		...
		foreach ($tab_array as $tab_elem) {
			if (
				isset($tab_elem['name']) && $tab_elem['name'] == $name
				&& $tab_elem['file'] == $file
			) {
	...
	function show()
	{
		...
		if ($this->showhelp) {
			...
			*/
			...
			$s .= "\n\t\t" . dPshowImage(
				'./images/icons/stock_help-16.png',
				'16',
				'16',
				$AppUI->_('Help')
			);
